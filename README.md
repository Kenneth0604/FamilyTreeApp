...
| 樹狀圖 | 以視角為中心、依世代分層:同一層裡現任配偶 / 伴侶黏成一個「單位」(保證相鄰、不會被插隊),再用重心法來回排序(孩子排到父母正下方、同父母的兄弟姊妹依長幼、先生的父母在左 / 太太的父母在右、前任盡量排在對方隔壁),最後以視角本人為圓心,用優先權法把每個單位靠向父母 / 孩子的中央,親近的人自然聚在一起;同一對父母先匯合到一個連接點再分岔到各孩子,連線是「血管」造型(父母端粗 → 中段細管 → 孩子端再變寬,轉角圓滑),血球沿管子由父母流向孩子(CSS 動畫,尊重 prefers-reduced-motion)。卡片顯示大頭照 / 姓名(小名)/ 稱謂,大小隨「戰力」縮放;配偶線是不流動的小血管:伴侶偏紫、離婚 / 前伴侶淡化成乾枯的虛線外框並標字。「✋ 編輯排版」模式下先點一下卡片選取、再按住拖曳調整位置(記在這台裝置,可一鍵重新排版);平常點卡片看詳細 |
...
| 戰力與讚讚人指數 | 戰力 = 地位(手動 0–10,這個人在家族裡的份量)+ 壽命加成(自動 0–5:60 歲花甲 +1、70 古稀 +2、80 耄耋 +3、90 鮐背 +4、100 人瑞 +5;在世者看現在年齡、已故者看享壽),總分 0–15 對應 邊緣人 → 不朽傳說。已故者填了逝世日期會顯示「享年 / 享壽 / 享耆壽 / 享嵩壽 N 歲」。遊戲角色式屬性算出「讚讚人指數」:有趣 / 有病 / 脾氣 / 聰明 / 學歷 / 有錢 / 顏值 / 廚藝 / 酒量 / 愛唸 / 運氣 / 固執 / 八卦…共 30 項可挑,拉滑桿評 0–10 分,顯示數值條與低 / 高分描述,並算出綜合評分、等級(S–D)、稱號、最強 / 最弱屬性 |
...
| `people` | name、nicknames(text[])、tags(text[])、gender(male / female / unspecified)、birth_date(文字,`YYYY` / `YYYY-MM` / `YYYY-MM-DD` 或 null)、birth_order(排行,1 = 老大;生日分不出長幼時用)、is_deceased、death_date(逝世日期,格式同 birth_date)、avatar_url、note、stats(jsonb,{屬性 id: 0–10})、power(0–10 戰力)、created_by / updated_by / updated_at |
...
src/
  ...
  lib/treeLayout.js   樹狀圖世代分層排版(夫妻單位 + 重心排序 + 優先權法定座標)
