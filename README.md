...
| 區塊 | 內容 |
| --- | --- |
| 帳號 | Email + 密碼(Supabase Auth)。首次登入建立新家族,或輸入 6 碼邀請碼 / 點邀請連結加入 |
| 家族群組 | 一個家族樹屬於一個 family;成員共同新增、編輯,每筆資料記錄 created_by / updated_by,詳細頁顯示「最後由誰編輯」 |
...
| 設定 | 切換視角(我是誰)、綁定帳號的真實身分、進階稱謂模式、邀請碼管理(分享 / 重新產生)、家族名稱、主題(粉粉 / 黑黑)、切換家族、離開家族 |
...
RLS:所有表以 `is_family_member(family_id)` 判斷讀寫;建立家族 / 加入 / 重新產生邀請碼透過 security definer 的 RPC。
