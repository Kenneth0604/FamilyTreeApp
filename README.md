...
| 樹狀圖 | 以視角為中心、依世代分層:同一層裡現任配偶 / 伴侶黏成一個「單位」(保證相鄰、不會被插隊),再用重心法來回排序(孩子排到父母正下方、同父母的兄弟姊妹依長幼、先生的父母在左 / 太太的父母在右、前任盡量排在對方隔壁),最後以視角本人為圓心,用優先權法把每個單位靠向父母 / 孩子的中央,親近的人自然聚在一起;同一對父母先匯合到一個連接點再分岔到各孩子,連線是「血管」造型(父母端粗 → 中段細管 → 孩子端再變寬,轉角圓滑),血球沿管子由父母流向孩子(CSS 動畫,尊重 prefers-reduced-motion)。卡片顯示大頭照 / 姓名(小名)/ 稱謂,大小隨「戰力」縮放;配偶線是不流動的小血管:伴侶偏紫、離婚 / 前伴侶淡化成乾枯的虛線外框並標字。「✋ 編輯排版」模式下先點一下卡片選取、再按住拖曳調整位置(記的是相對自動排版的位移、每個家族 × 排版方式一份,可一鍵重新排版);平常點卡片跳出快速選單:查看詳細,或以這個人為基準新增父母 / 子女 / 配偶 / 兄弟姊妹。「☰ 分層 / ☀ 放射」切換排版:放射排版以視角本人為圓心、依親等一圈圈往外(長輩上半圓、後代下半圓),父母 → 孩子直接以兩端寬、中段細的血管相連 |
...
| 合併家族樹 | 兩個家族各出一個人、指定關係(配偶 / 親子),透過 6 碼連結碼合併成一棵新的唯讀家族樹:A 家 editor 在設定頁選人 + 關係產生連結碼 → B 家 editor 輸入碼、預覽對方是誰、選自己這邊的人 → 建立 kind = merged 的新家族,兩邊所有成員(含之後加入的)都自動成為 viewer。合併樹沒有自己的資料,讀取時由 `get_merged_tree` RPC 把兩邊資料 + 橋接關係拼起來,在原家族編輯會同步反映;任一來源家族的 editor 可「解除合併」,只刪合併樹。**合併樹可以再合併**(來源家族的 editor 在合併樹的設定頁產生 / 輸入連結碼):合併樹 ⊕ 一般家族 → 把該家族加進來源;合併樹 ⊕ 合併樹 → 把對方所有來源與橋接併進輸入碼這一棵、對方那棵刪掉,所以合併樹永遠是扁平的多來源(`can_manage_family` / `family_source_ids` 輔助函式)。**同一個人**:每次資料更新都自動比對兩邊姓名 / 小名相符的人,頂端提示「XX 合併樹有一樣的人!」,到「同一個人?」頁逐組確認(附性別 / 出生年是否一致、父母是誰);確認後在合併樹裡把兩人併成一個節點(關係 / 紀事 / 寵物 / 小家庭都指向同一人、欄位取聯集,三個以上來源時可串接),雙方的親戚就串在同一張圖上;也可標記「不是同一人」或手動從兩個不同來源各選一人 |
| 小家庭 | 把同住的人圈成一戶(名稱 + 顏色 + 成員),樹狀圖用該色虛線描出一個**不規則外框**:成員卡片(含邊距)加上成員之間親子 / 配偶關係的「走廊」聯集起來、轉角磨圓(`lib/outline.js`),所以框會順著親緣關係彎,不會把中間無關的人也框進去;一個人可屬於多個小家庭。從樹狀圖的「⌂ 小家庭」進入管理 |
| 顯示方式 | 樹狀圖右上「👁 顯示方式」:預設(大家一樣大)/ 家庭地位(戰力越高卡片越大)/ 讚讚人指數(指數越高越大)/ 年齡(越年長越大,已故者用享壽)/ 政治立場(卡片頂端藍 / 綠 / 白色條)/ 自訂(挑幾項屬性,依平均分決定大小)。設定記在這台裝置 |
| 戰力與讚讚人指數 | 戰力 = 地位(手動 0–10,這個人在家族裡的份量)+ 壽命加成(自動 0–5:60 歲花甲 +1、70 古稀 +2、80 耄耋 +3、90 鮐背 +4、100 人瑞 +5;在世者看現在年齡、已故者看享壽),總分 0–15 對應 邊緣人 → 不朽傳說。已故者填了逝世日期(或只填大概歲數 `85` / `80-90`)會顯示「享年 / 享壽 / 享耆壽 / 享嵩壽 N 歲」,範圍取中間值算加成。遊戲角色式屬性算出「讚讚人指數」:有趣 / 有病 / 脾氣 / 聰明 / 學歷 / 有錢 / 顏值 / 廚藝 / 酒量 / 愛唸 / 運氣 / 固執 / 八卦…共 30 項可挑,拉滑桿評 0–10 分,顯示數值條與低 / 高分描述,並算出綜合評分、等級(S–D)、稱號、最強 / 最弱屬性 |
...
src/
  ...
  lib/treeLayout.js   樹狀圖排版:分層(夫妻單位 + 重心排序 + 優先權法定座標)與放射(BFS 樹 + 葉子數分扇形)
  lib/outline.js      小家庭不規則外框(長方形聯集 → 磨圓 SVG path)
